...
const team = (api.team = parseInt(
  query.get("team") || floor(random() * teams)
));
...
const renderer = (api.renderer = new WebGLRenderer({
  canvas: $canvas,
  antialias: true,
}));
...
const hit = (api.hit = new Mesh(
  new CylinderGeometry(0.1, 0.5, 1, 2 ** 5, 1, true),
  new MeshToonMaterial({
    transparent: true,
    opacity: 0.5,
    side: DoubleSide,
...
$canvas.addEventListener("pointermove", ({ clientX: x, clientY: y }) => {
  ...
  if (!data?.length) {
    return (hit.visible = hint.visible = false);
  }
  ...
  const l2 = reduce(
    (l2, ds) =>
      reduce(
        (l2, d) => {
          if (!ray.intersectSphere(dataToSphere(d, s), v)) {
            return l2;
          }
  ...
  if (t === team) {
    hit.material.color.setHex(0x000000);
  } else {
...
const dataToScene = (api.dataToScene = (to) => {
  ...
  map(() => [], data.length ? data : (data.length = teams) && range(data), 0);
...
setTimeout(() => {
  const itemsToShow = olta.getAll("forms");
  // const initialStateItems = itemsToShow.filter(item => typeof item._id === 'number');
  let showQueue = [];

  dataToScene(showQueue);

  itemsToShow.map((item, index) => {
    setTimeout(() => {
      showQueue = [...showQueue, item];
